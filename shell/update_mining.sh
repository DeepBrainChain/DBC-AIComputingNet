...
mining_file=dbc_${os_name}_mining_${latest_version}.tar.gz

wget -P ./update_temp http://111.44.254.179:22244/dbc/${mining_file}
if [ $? -ne 0 ]; then
  echo "wget dbc package failed!"
  exit 1
fi

tar -zxvf ./update_temp/${mining_file} -C ./update_temp/
