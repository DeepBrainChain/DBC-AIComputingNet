...
for ((i=1;i<=$dir_num;i++)); do
  ...
  sed -i "/net_type=/c net_type=netcongtu" $install_dir/conf/core.conf
  sed -i "/net_flag=/c net_flag=0xF1E1B0E8" $install_dir/conf/core.conf
  ...
  echo "dbc_chain_domain=congtuhttps.dbcwallet.io:7788" >> $install_dir/conf/core.conf
