...
cp -rf ./update_cache/dbc_client_node/shell/crontab /home
...
for ((i=1;i<=$dir_num;i++)); do
  ...
  # image_server
  old_image_servers=$(cat $install_dir/conf/core.conf | grep "image_server=")

  cp -fp $install_dir/conf/peer.conf ./update_cache/dbc_client_node/peer_old.conf

  new_net_listen_ip=$(cat ./update_cache/dbc_client_node/conf/core.conf | grep "net_listen_ip=" | awk -F '=' '{print $2}')
  new_net_listen_port=$(cat ./update_cache/dbc_client_node/conf/core.conf | grep "net_listen_port=" | awk -F '=' '{print $2}')
  new_http_ip=$(cat ./update_cache/dbc_client_node/conf/core.conf | grep "http_ip=" | awk -F '=' '{print $2}')
  ...
  for svr in ${old_image_servers}; do
    echo $svr >> $install_dir/conf/core.conf
  done
