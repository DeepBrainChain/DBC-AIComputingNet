...
cp -r ./install_cache/dbc_baremetal_node/*  $install_dir/

# install_dbc_baremetal
echo "installing..."
...
if [[ -d "/home/crontab" && -f "/home/crontab/dbc_process.conf" ]]; then
  rm -rf ./install_cache/dbc_baremetal_node/dbc_dir_old.conf
  cp /home/crontab/dbc_process.conf ./install_cache/dbc_baremetal_node/dbc_dir_old_cache.conf
  for fline in $(cat ./install_cache/dbc_baremetal_node/dbc_dir_old_cache.conf); do
    if [[ $fline == *=* ]]; then
      ffullpath=$(echo $fline | awk -F'=' '{print $2}' | awk -F'"' '{print $2}')
      echo $ffullpath >>./install_cache/dbc_baremetal_node/dbc_dir_old.conf
    else
      echo $fline >>./install_cache/dbc_baremetal_node/dbc_dir_old.conf
    fi
  done
  rm -rf ./install_cache/dbc_baremetal_node/dbc_dir_old_cache.conf
fi

if [[ -d "/home/crontab" && -f "/home/crontab/dbc_dir.conf" ]]; then
  rm -rf ./install_cache/dbc_baremetal_node/dbc_dir_old.conf
  cp /home/crontab/dbc_dir.conf ./install_cache/dbc_baremetal_node/dbc_dir_old_cache.conf
  for fline in $(cat ./install_cache/dbc_baremetal_node/dbc_dir_old_cache.conf); do
    if [[ $fline == *=* ]]; then
      ffullpath=$(echo $fline | awk -F'=' '{print $2}' | awk -F'"' '{print $2}')
      echo $ffullpath >>./install_cache/dbc_baremetal_node/dbc_dir_old.conf
...
cp -rf ./install_cache/dbc_baremetal_node/shell/crontab /home
if [[ -f "./install_cache/dbc_baremetal_node/dbc_dir_old.conf" ]]; then
  cat ./install_cache/dbc_baremetal_node/dbc_dir_old.conf > /home/crontab/dbc_dir.conf
fi
...
echo "install dbc baremetal node success!"
