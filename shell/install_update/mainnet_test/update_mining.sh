...
for ((i=1;i<=$dir_num;i++)); do
  ...
  new_net_listen_ip=$(cat ./update_cache/dbc_mining_node/conf/core.conf | grep "net_listen_ip=" | awk -F '=' '{print $2}')
  new_net_listen_port=$(cat ./update_cache/dbc_mining_node/conf/core.conf | grep "net_listen_port=" | awk -F '=' '{print $2}')
  new_http_ip=$(cat ./update_cache/dbc_mining_node/conf/core.conf | grep "http_ip=" | awk -F '=' '{print $2}')
