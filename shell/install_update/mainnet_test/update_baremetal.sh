...
node_file=dbc_baremetal_node_${latest_version}.tar.gz
...
if [[ -d "/home/crontab" && -f "/home/crontab/dbc_process.conf" ]]; then
  rm -rf ./update_cache/dbc_baremetal_node/dbc_dir_old.conf
  cp /home/crontab/dbc_process.conf ./update_cache/dbc_baremetal_node/dbc_dir_old_cache.conf
  for  fline  in  `cat ./update_cache/dbc_baremetal_node/dbc_dir_old_cache.conf`; do
    if [[ $fline == *=* ]]; then
      ffullpath=`echo $fline|awk -F'=' '{print $2}'|awk -F'"' '{print $2}'`
      echo $ffullpath >> ./update_cache/dbc_baremetal_node/dbc_dir_old.conf
    else
      echo $fline >> ./update_cache/dbc_baremetal_node/dbc_dir_old.conf
    fi
  done
  rm -rf ./update_cache/dbc_baremetal_node/dbc_dir_old_cache.conf
fi

if [[ -d "/home/crontab" && -f "/home/crontab/dbc_dir.conf" ]]; then
  rm -rf ./update_cache/dbc_baremetal_node/dbc_dir_old.conf
  cp /home/crontab/dbc_dir.conf ./update_cache/dbc_baremetal_node/dbc_dir_old_cache.conf
  for  fline  in  `cat ./update_cache/dbc_baremetal_node/dbc_dir_old_cache.conf`; do
    if [[ $fline == *=* ]]; then
      ffullpath=`echo $fline|awk -F'=' '{print $2}'|awk -F'"' '{print $2}'`
      echo $ffullpath >> ./update_cache/dbc_baremetal_node/dbc_dir_old.conf
...
cp -rf ./update_cache/dbc_baremetal_node/shell/crontab /home
if [[ -f "./update_cache/dbc_baremetal_node/dbc_dir_old.conf" ]]; then
  cat ./update_cache/dbc_baremetal_node/dbc_dir_old.conf > /home/crontab/dbc_dir.conf
fi
...
for ((i=1;i<=$dir_num;i++)); do
  ...
  old_dbc_monitor_server=$(cat $install_dir/conf/core.conf | grep "dbc_monitor_server=" | awk -F '=' '{print $2}')
  old_miner_monitor_server=$(cat $install_dir/conf/core.conf | grep "miner_monitor_server=" | awk -F '=' '{print $2}')

  cp -fp $install_dir/conf/peer.conf ./update_cache/dbc_baremetal_node/peer_old.conf

  new_net_listen_ip=$(cat ./update_cache/dbc_baremetal_node/conf/core.conf | grep "net_listen_ip=" | awk -F '=' '{print $2}')
  new_net_listen_port=$(cat ./update_cache/dbc_baremetal_node/conf/core.conf | grep "net_listen_port=" | awk -F '=' '{print $2}')
  new_http_ip=$(cat ./update_cache/dbc_baremetal_node/conf/core.conf | grep "http_ip=" | awk -F '=' '{print $2}')
  new_http_port=$(cat ./update_cache/dbc_baremetal_node/conf/core.conf | grep "http_port=" | awk -F '=' '{print $2}')

  cp -rfp ./update_cache/dbc_baremetal_node/dbc $install_dir/
  cp -rfp ./update_cache/dbc_baremetal_node/shell $install_dir/
  cp -rfp ./update_cache/dbc_baremetal_node/conf $install_dir/

  sed -i "/net_type=/c net_type=mainnet_test" $install_dir/conf/core.conf
  sed -i "/net_flag=/c net_flag=0xF1E1B1F9" $install_dir/conf/core.conf
  ...
  if [ -n "$old_miner_monitor_server" ]; then
    echo "miner_monitor_server=${old_miner_monitor_server}" >> $install_dir/conf/core.conf
  fi
  sed -i '/dbc_chain_domain=/d' $install_dir/conf/core.conf
  echo "dbc_chain_domain=infotest2.dbcwallet.io:7778" >> $install_dir/conf/core.conf

  cat ./update_cache/dbc_baremetal_node/peer_old.conf >> $install_dir/conf/peer.conf
...
echo "update all dbc baremetal nodes success!"
