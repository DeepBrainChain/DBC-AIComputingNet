    ...
    namespace dbc
    {
        ...
        public:
            ...
            void format_output()
            {
                ...
                for(; it != task_info_list.end(); it++)
                {
                    ...
                    {
                        cout << "task id: " << it->task_id << "     create_time: " << time_util::time_2_str(it->create_time) << " result: " << it->result << endl;
                    }
                }
            }
        };
        ...
        public:
            ...
            void format_output()
            {
                ...
                {
                    printer << matrix::core::init << i << it->task_id << time_util::time_2_str(it->create_time) << to_training_task_status_string(it->status) << matrix::core::endl;
                }
            }
        };
            ...
            void format_output(matrix::service_core::get_peers_flag flag)
            {
                ...
                for(; it != v->peer_nodes_list.end(); ++it)
                {
                    if(matrix::service_core::flag_active == flag)
                    {
                        ...
                        //    }
                        //}
                        printer << matrix::core::init << string_util::fuzz_ip(it->addr.ip) << it->addr.port << (int32_t) it->node_type
                                //<< svc_list
                                << it->peer_node_id << matrix::core::endl;
                    }
                    else if(matrix::service_core::flag_global == flag)
                    {
                        printer << matrix::core::init << string_util::fuzz_ip(it->addr.ip) << it->addr.port << (int32_t) it->node_type << matrix::service_core::net_state_2_string(it->net_st) << (it->peer_node_id.empty()?"N/A":it->peer_node_id) << matrix::core::endl;
                    }
                }
            }
        ...
        public:
            ...
            enum
            {
                TIMESTAMP_STR_LENGTH = 30, IMAGE_HASH_STR_PREFIX_LENGTH = 12, IMAGE_HASH_STR_MAX_LENGTH = 64,
        ...
        public:
            cmd_show_req():op(OP_SHOW_UNKNOWN), sort("gpu")
            {
