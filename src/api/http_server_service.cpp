    ...
    namespace core
    {
        ...
        void http_server_service::start_http_server()
        {
            ...
            for (unsigned int i = 0; i < ARRAYLEN(uri_prefixes); i++)
            {
                register_http_handler(
                        REST_API_URI + uri_prefixes[i].m_prefix,
        ...
        void http_server_service::http_request_cb(struct evhttp_request *req, void *arg)
        {
            ...
            if (time_span_ms.count() < MIN_INIT_HTTP_SERVER_TIME)
            {
                ...
                hreq->reply_comm_rest_err(HTTP_OK, RPC_REQUEST_INTERRUPTED,
                                          "Request is interrupted.Try again later.");
            ...
            if (str_uri.substr(0, REST_API_URI.size()) != REST_API_URI)
            {
                ...
                hreq->reply_comm_rest_err(HTTP_BADREQUEST, RPC_INVALID_REQUEST,
                                          "The api version is invalid. Current "
