...
namespace matrix
{
    namespace core
    {
        ...
        void http_server_service::start_http_server()
        {
            ...
            {
                register_http_handler(REST_API_URI + uri_prefixes[i].m_prefix, uri_prefixes[i].m_exact_match,
        ...
        void http_server_service::stop_http_server()
        {
            ...
            if( m_event_base_ptr )
            {
                ...
                if( m_thread_result.valid() && m_thread_result.wait_for(
                        std::chrono::milliseconds(2000)) == std::future_status::timeout )
                {
        ...
        void http_server_service::http_request_cb( struct evhttp_request* req, void* arg )
        {
            ...
            std::unique_ptr<http_request> hreq(
                    new http_request(req, reinterpret_cast<http_server_service*>(arg)->get_event_base_ptr()));

            LOG_INFO << "Received a " << hreq->request_method_string(
                        hreq->get_request_method()) << ", request for " << hreq->get_uri() << " from " << hreq->get_peer().get_ip();
        ...
        {
            // rest ip addr
            std::string conf_rest_ip = options.count(
                    "rest_ip") ? options["rest_ip"].as<std::string>() : CONF_MANAGER->get_rest_ip();
            ...
            // rest port
            std::string conf_rest_port = options.count(
                    "rest_port") ? options["rest_port"].as<std::string>() : CONF_MANAGER->get_rest_port();
        ...
        void http_server_service::register_http_handler( const std::string& prefix, bool exact_match,
