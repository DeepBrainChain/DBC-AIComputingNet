    ...
    namespace core
    {
        ...
        void http_server_service::start_http_server()
        {
            ...
            for (unsigned int i = 0; i < ARRAYLEN(uri_prefixes); i++) {
                register_http_handler(REST_API_URI + uri_prefixes[i].m_prefix,
        ...
        void http_server_service::http_request_cb(struct evhttp_request* req, void* arg)
        {
            ...
            std::shared_ptr<http_request> hreq(new http_request(req,
                                                                reinterpret_cast<http_server_service*>(arg)->get_event_base_ptr()));

            LOG_DEBUG << "Received a " << hreq->request_method_string(hreq->get_request_method()) << ", request for "
                      << hreq->get_uri() << " from " << hreq->get_peer().get_ip();
            ...
            reinterpret_cast<http_server_service*>(arg)->on_http_request_event(i->m_handler, hreq, path);
...
//            }
        ...
        void http_server_service::register_http_handler(const std::string& prefix,
                                                        bool exact_match,
