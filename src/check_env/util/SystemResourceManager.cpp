...
std::string SystemResourceManager::os_name() {
    std::string os_name = run_shell(
        "cat /etc/os-release | grep VERSION | awk -F '=' '{print $2}'| tr -d "
        "'\"'");
...
int32_t SystemResourceManager::physical_cpu() {
    std::string s_physical_cpu =
        run_shell("cat /proc/cpuinfo| grep \"physical id\"| sort| uniq| wc -l");
...
int32_t SystemResourceManager::cpu_cores_per_physical() {
    std::string s_cpu_cores_per_physical =
        run_shell("cat /proc/cpuinfo| grep \"cpu cores\"| uniq");
...
int32_t SystemResourceManager::cpu_siblings_per_physical() {
    std::string s_cpu_siblings_per_physical =
        run_shell("cat /proc/cpuinfo | grep 'siblings' | uniq");
    s_cpu_siblings_per_physical =
        util::rtrim(s_cpu_siblings_per_physical, '\n');
    ...
    int32_t num_cpu_siblings_per_physical =
        atoi(s_cpu_siblings_per_physical.c_str());
...
void SystemResourceManager::init_gpu() {
    std::string cmd =
        "lspci -nnv |grep NVIDIA |awk '{print $2\",\"$1}' |tr \"\n\" \"|\"";
...
int64_t SystemResourceManager::mem_total() {
    std::string mem_total =
        run_shell("free -k | grep \"Mem:\" | awk '{print $2}'");
...
int64_t SystemResourceManager::mem_used() {
    std::string mem_used =
        run_shell("free -k | grep \"Mem:\" | awk '{print $3}'");
...
int64_t SystemResourceManager::mem_free() {
    std::string mem_free =
        run_shell("free -k | grep \"Mem:\" | awk '{print $4}'");
    mem_free = util::rtrim(mem_free, '\n');
    return atoll(mem_free.c_str());
}

int64_t SystemResourceManager::mem_buff_cache() {
    std::string mem_buff_cache =
        run_shell("free -k | grep \"Mem:\" | awk '{print $6}'");
...
int64_t SystemResourceManager::mem_available() {
    std::string mem_available =
        run_shell("free -k | grep \"Mem:\" | awk '{print $7}'");
    mem_available = util::rtrim(mem_available, '\n');
    return atoll(mem_available.c_str());
}

void SystemResourceManager::init_disk(const std::string& path) {
    m_disk.type =
        disk_type(path) == "SSD" ? DiskType::DT_SSD : DiskType::DT_HDD;
    m_disk.total = disk_total(path);
    m_disk.available = disk_available(path);
}

std::string SystemResourceManager::disk_type(const std::string& path) {
    std::string cmd1 =
        "df -l -m " + path +
        " | tail -1 | awk '{print $1}' | awk -F\"/\" '{print $3}'";
    std::string disk = run_shell(cmd1.c_str());
    disk = util::rtrim(disk, '\n');
    std::string cmd2 =
        "lsblk -o name,rota | grep " + disk +
        " | awk '{if($2==\"1\")print \"HDD\"; else print \"SSD\"}'";
...
void SystemResourceManager::print_cpu() {
    ...
    std::cout << "total_cores: " << m_cpu.total_cores() << std::endl
              << "sockets: " << m_cpu.sockets
              << ", cores_per_socket: " << m_cpu.cores_per_socket
              << ", threads_per_core: " << m_cpu.threads_per_core << std::endl;
}
...
void SystemResourceManager::print_gpu() {
    ...
    for (auto& it : m_gpu) {
        std::cout << it.first << " : "
                  << "bus=" << DeviceGpu::parse_bus(it.first) << ","
                  << "slot=" << DeviceGpu::parse_slot(it.first) << ","
                  << "function=" << DeviceGpu::parse_function(it.first)
                  << std::endl;
...
void SystemResourceManager::print_mem() {
    std::cout << "mem:" << std::endl
              ...
              << ", available: " << size_to_string(m_mem.available, 1024 * 1024)
              << std::endl;
}
...
void SystemResourceManager::print_disk() {
    std::cout << "disk:" << std::endl
              ...
              << ", available: "
              << size_to_string(m_disk.available, 1024 * 1024) << std::endl;
}
