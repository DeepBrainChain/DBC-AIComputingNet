...
static uint32_t max_gpu_count = 1024;
// 保留 32G 内存
static int64_t g_reserved_memory = 32 * 1024 * 1024;
...
static std::string createXmlStr(const std::string &uuid,
                                const std::string &domain_name, int64_t memory,
                                int32_t cpunum, int32_t sockets, int32_t cores,
                                int32_t threads, const std::string &vedio_pci,
                                const std::string &image_file,
                                const std::string &disk_file, int32_t vnc_port,
                                const std::string &vnc_pwd,
    ...
    tinyxml2::XMLElement* node_suspend_to_mem =
    doc.NewElement("suspend-to-mem");
    node_suspend_to_mem->SetAttribute("enabled", "no");
    node_pm->LinkEndChild(node_suspend_to_mem);
    tinyxml2::XMLElement* node_suspend_to_disk =
    doc.NewElement("suspend-to-disk");
    ...
    */
    ...
    agent_source_node->SetAttribute("path",
                                    ("/tmp/channel_" + uuid + ".sock").c_str());
    ...
    // 如果想要收集内存统计信息，需要开启memballon，安装virtio驱动程序。有时还需要设置时间间隔<stats
    // period='10' />
    ...
    tinyxml2::XMLElement *node_qemu_commandline =
        doc.NewElement("qemu:commandline");
    if (!is_windows) {
...
std::string CreateDomainXML(const std::string &domain_name,
                            const std::string &system_disk_path,
                            const std::string &data_disk_path, int32_t sockets,
                            int32_t cores_per_socket, int32_t threads_per_core,
                            const std::string &vga_pci, int64_t mem,
                            int64_t data_file_size, int32_t vnc_port,
                            const std::string &vnc_pwd,
                            const std::vector<std::string> &multicast,
                            bool is_windows = false, bool uefi = false) {
    bool ret = true;
    ...
    std::cout << "image: " << system_disk_path << std::endl;

    // cpu
    long cpuNumTotal = sockets * cores_per_socket * threads_per_core;
    std::cout << "cpu: " << cpuNumTotal << "(" << sockets << ", "
    ...
    // 复制一份镜像（系统盘）
    auto pos = system_disk_path.find('.');
    std::string to_image_name = system_disk_path;
    std::string to_ext;
    if (pos != std::string::npos) {
        to_image_name = system_disk_path.substr(0, pos);
        to_ext = system_disk_path.substr(pos + 1);
    }
    std::string to_image_path =
        to_image_name + "_" + domain_name + "." + to_ext;
    std::cout << "image_file: " << to_image_path << std::endl;
    boost::system::error_code error_code;
    boost::filesystem::copy_file(
        system_disk_path, to_image_path,
        boost::filesystem::copy_option::overwrite_if_exists, error_code);
    if (error_code) {
        std::cout << "copy image file error: " << error_code.message()
                  << std::endl;
        return "";
    }
    ...
    std::cout << "data_file: " << data_disk_path << std::endl;
    if (boost::filesystem::exists(data_disk_path)) {
        remove(data_disk_path.c_str());
    }
    std::string cmd_create_img = "qemu-img create -f qcow2 " + data_disk_path +
                                 " " + std::to_string(data_file_size) + "G";
    ...
    std::string xml_content = createXmlStr(
        buf_uuid, domain_name, memoryTotal, cpuNumTotal, sockets,
        cores_per_socket, threads_per_core, vga_pci, to_image_path,
        data_disk_path, vnc_port, vnc_pwd, multicast, is_windows, uefi);
    return xml_content;
}

//////////////////////////////////////////////////////////////////////////////////
void transform_port(const std::string &domain_name,
    ...
    run_shell(cmd.c_str());
    cmd = "sudo iptables -t nat -A " + chain_name + " -p tcp --destination " +
          public_ip + " --dport " + transform_port +
          " -j DNAT --to-destination " + vm_local_ip +
          (is_windows ? ":3389" : ":22");
    run_shell(cmd.c_str());
...
void add_port_mapping(const std::string &domain_name,
                      const std::string &public_ip, const std::string &s_port,
                      const std::string &d_port, const std::string &vm_local_ip,
                      const std::string &protocol) {
    std::string chain_name = "check_chain_" + domain_name;
    std::string cmd = "sudo iptables -t nat -A " + chain_name + " -p " +
                      protocol + " --destination " + public_ip + " --dport " +
                      s_port + " -j DNAT --to-destination " + vm_local_ip +
                      ":" + d_port;
    run_shell(cmd.c_str());
}
...
void delete_image_file(const std::string &task_id,
    ...
    std::string real_image_path = real_image_name + "_" + task_id + "." + ext;
...
void delete_iptable(const std::string &domain_name,
                    const std::string &public_ip,
                    const std::string &transform_port,
    ...
    if (!vm_local_ip.empty()) {
        cmd =
            "sudo iptables -t nat -D PREROUTING --protocol tcp --destination " +
            public_ip + " --destination-port " + transform_port +
            " --jump DNAT --to-destination " + vm_local_ip + ":22";
        cmd += " && sudo iptables -t nat -D PREROUTING -p tcp --dport " +
               transform_port + " -j DNAT --to-destination " + vm_local_ip +
               ":22";
        auto pos = vm_local_ip.rfind('.');
        std::string ip = vm_local_ip.substr(0, pos) + ".1";
        cmd += " && sudo iptables -t nat -D POSTROUTING -p tcp --dport " +
               transform_port + " -d " + vm_local_ip + " -j SNAT --to " + ip;
        cmd +=
            " && sudo iptables -t nat -D PREROUTING -p tcp -m tcp --dport "
            "20000:60000 -j DNAT --to-destination " +
            vm_local_ip + ":20000-60000";
        cmd +=
            " && sudo iptables -t nat -D PREROUTING -p udp -m udp --dport "
            "20000:60000 -j DNAT --to-destination " +
            vm_local_ip + ":20000-60000";
        cmd += " && sudo iptables -t nat -D POSTROUTING -d " + vm_local_ip +
               " -p tcp -m tcp --dport 20000:60000 -j SNAT --to-source " +
               public_ip;
        cmd += " && sudo iptables -t nat -D POSTROUTING -d " + vm_local_ip +
               " -p udp -m udp --dport 20000:60000 -j SNAT --to-source " +
               public_ip;
        run_shell(cmd.c_str());
    }

    std::cout << "delete iptable: " << public_ip << ", " << transform_port
              << ", " << vm_local_ip << std::endl;
}
...
void test_kvm(int argc, char **argv) {
    ...
    std::string system_disk_path, data_disk_path;
    ...
    if (memory_total <= g_reserved_memory) {
        std::cout << "memory total size too short" << std::endl;
        return;
    }
    int64_t memory_size = memory_total * 0.1f;

    std::string cmd = "lspci |grep NVIDIA | awk '{print $1}' |tr \"\n\" \"|\"";
    std::string vga_gpu = run_shell(cmd.c_str());

    uint32_t gpu_count = SystemResourceMgr::instance().GetGpu().size();
    uint32_t gpu_used = gpu_count;

    int64_t specify_data_size = 0;
...

    {
        ...
        opts.add_options()
            ("ssh_port", boost::program_options::value<std::string>(),
                "specify ssh port in ubuntu or rdp port in windows")
            ("localip", boost::program_options::value<std::string>(),
                "remove vm and delete local ip in iptables")
            ("delete", "delete virtual machine")
            ("image", boost::program_options::value<std::string>(),
                "specify the absolute image path used to create vm")
            ...
            ("uefi", "uefi loader")
            ("max_gpus", boost::program_options::value<uint32_t>(),
                "max gpu count")
            ("multicast", boost::program_options::value<std::string>(),
                "multicast address")
            ("gpu_bus_ids", boost::program_options::value<std::string>(),
                "the tuple bus:device.function gpu PCI identifier")
            ("cpu_count", boost::program_options::value<int32_t>(),
                "specify cpu count")
            ("port_mapping",
                boost::program_options::value<std::vector<std::string>>(),
                "port mapping")
            ("username", boost::program_options::value<std::string>(),
                "login user name")
            ("password", boost::program_options::value<std::string>(),
                "login user password")
            ("data_size", boost::program_options::value<int64_t>(),
                "data disk size(GB), and will be used "
                "directly without verification")
            ("data_disk", boost::program_options::value<std::string>(),
                "specify the absolute data disk path");
        // clang-format on
        ...
        try {
            ...
            if (vm.count("localip")) {
                // delete_domain(domain_name, image_name, public_ip, ssh_port,
                //               vm["localip"].as<std::string>());
                std::shared_ptr<virDomainImpl> domain =
                    virt.openDomain(domain_name.c_str());
                if (domain) {
            ...
            if (vm.count("vnc_port")) {
                ...
                if (port < 5900 || port > 6000) {
                    std::cout << "vnc port should between 5900 and 6000"
                              << std::endl;
                    return;
                }
                vnc_port = port;
            ...
            if (vm.count("max_gpus")) {
                max_gpu_count = vm["max_gpus"].as<uint32_t>();
                gpu_used = std::min(gpu_count, max_gpu_count);
            }
            if (vm.count("multicast")) {
                multicast = SplitStr(vm["multicast"].as<std::string>(), ',');
                if (multicast.empty()) {
                    std::cout << "multicast address format: 230.0.0.1:5558"
                              << std::endl;
                    return;
                }
                for (const auto &address : multicast) {
                    ...
                    boost::asio::ip::tcp::endpoint ep(
                        ...
                        atoi(vecSplit[1].c_str()));
                    if (!ep.address().is_multicast()) {
                        std::cout << "address is not a multicast address"
                                  << std::endl;
                        return;
                    }
                }
            ...
            if (vm.count("gpu_bus_ids")) {
                ...
                for (const auto &busId : busIds) {
                    ...
                    if (run_shell(cmd.c_str()).empty()) {
                        std::cout << "invalid gpu PCI identifier: " << busId
                                  << std::endl;
                        return;
                    }
                    if (bus_ids_count > 0) gpu_bus_ids += "|";
                    gpu_bus_ids += busId;
                    ++bus_ids_count;
                    std::vector<std::string> vectemp = SplitStr(busId, ':');
                    if (buses.count(vectemp[0]) == 0) buses.insert(vectemp[0]);
                }
                if (gpu_bus_ids.empty()) {
                    ...
                    vga_gpu = gpu_bus_ids;
                    gpu_used = std::min(gpu_used, (uint32_t)buses.size());
                }
            }
            ...
            if (vm.count("data_size")) {
                specify_data_size = vm["data_size"].as<int64_t>();
            }
            if (vm.count("data_disk")) {
                data_disk_path = vm["data_disk"].as<std::string>();
            }
    ...
    if (!boost::filesystem::exists(image_path, error_code) || error_code) {
        image_path = boost::filesystem::path("/data/" + image_name);
        if (!boost::filesystem::exists(image_path, error_code) || error_code) {
            std::cout << "image does not exist" << std::endl;
            return;
        }
    }
    if (!boost::filesystem::is_regular_file(image_path, error_code) ||
        error_code) {
        std::cout << "image is not a regular file" << std::endl;
        return;
    }
    system_disk_path = boost::filesystem::absolute(image_path).string();

    if (data_disk_path.empty()) {
        data_disk_path = "/data/data_1_" + domain_name + ".qcow2";
    } else {
        boost::filesystem::path data_directory(data_disk_path);
        SystemResourceMgr::instance().init_disk(
            data_directory.parent_path().string());
    }

    int64_t disk_total_size =
        SystemResourceMgr::instance().GetDisk().total / 1024;  // GB
    if (disk_total_size <= 350) {
        std::cout << "disk total size too short" << std::endl;
        return;
    }
    int64_t disk_size = (disk_total_size - 350) * 0.1f;

    if (gpu_used > 0) {
        memory_size = (memory_total - g_reserved_memory) * gpu_used / gpu_count;
        disk_size = (disk_total_size - 350) * gpu_used / gpu_count;
    }
    if (specify_data_size > 0) {
        disk_size = specify_data_size;
    }
    ...
    try {
        ...
        std::string xml_content = CreateDomainXML(
            domain_name, system_disk_path, data_disk_path, sockets, cores,
            threads, vga_gpu, memory_size, disk_size, vnc_port, vnc_pwd,
            multicast, is_windows, uefi_load);
        ...
        if (!domain) {
            printLastError();
            delete_image_file(domain_name, system_disk_path);
        ...
        if (domain->startDomain() < 0) {
            std::cout << "start domain xml" << std::endl;
            printLastError();
            domain->deleteDomain();
        ...
        while (vm_local_ip.empty() && try_count++ < 200) {
            if (try_count == 100) {
                ...
                // std::string tmp = run_shell(
                //     ("virsh reboot --mode agent " + domain_name).c_str());
                // std::cout << "virsh reboot --mode agent " << domain_name
                //           << " return : " << tmp << std::endl;
            ...
            sleep(3);
            std::cout << "get vm_local_ip try_count: " << try_count
                      << std::endl;
        ...
        transform_port(domain_name, public_ip, ssh_port, vm_local_ip,
                       is_windows);
        for (const auto &mapping : portmapping) {
            ...
            if (vec1[0].find("tcp") != std::string::npos)
                add_port_mapping(domain_name, public_ip, sport, dport,
                                 vm_local_ip, "tcp");
        ...
        while (succ < 0 && try_count++ < 100) {
            sleep(3);
            std::cout << "set_vm_password try_count: " << try_count
                      << std::endl;
            succ =
                domain->setDomainUserPassword(vm_user.c_str(), vm_pwd.c_str());
            if (succ < 0) {
                printLastError();
            }
        }
        ...
        std::cout << "set vm password successful, user:" << vm_user
                  << ", pwd:" << vm_pwd << std::endl;

        std::cout << "vnc port:" << vnc_port << " , password:" << vnc_pwd
                  << std::endl;
