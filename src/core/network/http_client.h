...
inline raii_bufferevent obtain_evhttp_bev(struct event_base* base, SSL *ssl)
{
    //raii_bufferevent bev;
    if (ssl != nullptr)
    {
#ifdef __linux__
        auto bev = raii_bufferevent(bufferevent_openssl_socket_new(base, -1, ssl, BUFFEREVENT_SSL_CONNECTING, BEV_OPT_CLOSE_ON_FREE | BEV_OPT_DEFER_CALLBACKS));
        if (!bev.get())
            throw std::runtime_error("create connection failed");
        bufferevent_enable(bev.get(), EV_READ | EV_WRITE | EV_PERSIST);
    ...
    {
        auto bev = raii_bufferevent(bufferevent_socket_new(base, -1, BEV_OPT_CLOSE_ON_FREE));
        if (!bev.get())
            throw std::runtime_error("create connection failed");
        return bev;
    }
...
inline bufferevent * obtain_evhttp_bev2(struct event_base* base, SSL *ssl)
{
    ...
    if (ssl != nullptr)
    {
        ...
        //bev = raii_bufferevent(bufferevent_openssl_socket_new(base, -1, ssl, BUFFEREVENT_SSL_CONNECTING, BEV_OPT_CLOSE_ON_FREE | BEV_OPT_DEFER_CALLBACKS));
        if (!bev)
            throw std::runtime_error("create connection failed");
