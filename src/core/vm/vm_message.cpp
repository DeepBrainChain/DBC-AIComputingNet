...
* description    :   vn message definition
* date                  :    2021.04.27
* author            :   barry
**********************************************************************************/
...
namespace matrix
{
    namespace core
    {
        ...
        std::string vm_config::to_string()
        {
            ...
            for (auto it = host_config.devices.begin(); it != host_config.devices.end(); it++)
            {
                ...
                json_device.AddMember("PathInContainer", STRING_REF(it->path_in_vm), allocator);
        ...
        std::string update_vm_config::update_to_string()
        {
            rapidjson::Document document;
            rapidjson::Document::AllocatorType& allocator = document.GetAllocator();
        ...
        void vm_create_resp::from_string(const std::string & buf)
        {
            try
            {
                //parse resp
                ...
                {
                    LOG_ERROR << "parse vm_create_resp file error:" << GetParseError_En(doc.GetParseError());
                    return ;
                }
                ...
                {
                    LOG_ERROR << "vm client create vm resp has no id field";
                    return;
                }

                rapidjson::Value &id = doc["Id"];
                this->vm_id = id.GetString();
            ...
            {
                LOG_ERROR << "vm client create vm resp exception";
            }

        }

        void vm_inspect_response::from_string(const std::string & buf)
        {
            try
            {
                rapidjson::Document doc;
                ...
                if (doc.Parse<0>(buf.c_str()).HasParseError())
                {
                    LOG_ERROR << "parse vm_inspect_response file error:" << GetParseError_En(doc.GetParseError());
                    return;
                }
                ...
                {
                    LOG_ERROR << "vm client inspect vm resp has no id state";
                    return;
                }
            ...
            {
                LOG_ERROR << "vm client inspect vm resp exception";
            }

        }

        void vm_info::from_string(const std::string & buf)
        {
            try
            {
                rapidjson::Document doc;
            ...
            {
                LOG_ERROR << "vm client get docker info exception";
            }
        }
