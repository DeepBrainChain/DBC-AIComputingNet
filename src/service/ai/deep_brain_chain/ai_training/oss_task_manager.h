...
namespace ai
{
    ...
    { 
        ...
        public:
            oss_task_manager(std::shared_ptr<idle_task> &idle_task_ptr);
        ...
        private:
            ...
            std::shared_ptr<idle_task> m_idle_task_ptr = nullptr;

            //when get idle task failed from oss, try again after m_get_idle_task_interval
            const int64_t  m_get_idle_task_interval = 5 * 60 * 1000;
            int64_t m_next_update_interval = 0;

            //try update idle task by day.
            const int64_t  m_get_idle_task_day_interval = 24 *60 * 60 * 1000; //24h
