...
namespace ai
{
    namespace dbc
    {
        ...
        int32_t ai_power_provider_service::init_conf()
        {
            ...
            m_oss_task_mng = std::make_shared<oss_task_manager>(m_container_client);
        ...
        int32_t ai_power_provider_service::on_training_task_timer(std::shared_ptr<core_timer> timer)
        {
            if (0 == m_queueing_tasks.size())
            {
                m_oss_task_mng->exec_idle_task(std::bind(&ai_power_provider_service::get_container_config, this, std::placeholders::_1));
                LOG_DEBUG << "training queuing task is empty";
                return E_SUCCESS;
            }
