...
namespace ai
{
    namespace dbc
    {
        ...
        int32_t ai_power_provider_service::auth_task(std::shared_ptr<ai_training_task> task, bool is_report_cycle)
        {
            ...
            if (m_bill_client != nullptr)
            {
                ...
                if (nullptr == resp)
                {
                    ...
                    m_task_timer_map[task->task_id] = DEFAULT_AUTH_REPORT_CYTLE * 60 * 1000*1000;
                    return E_SUCCESS;
                }
                    ...
                    && (task->status ==task_queueing || task->status ==task_running || task->status == task_pulling_image))
                {
                    ...
                    m_task_timer_map[task->task_id] = resp->report_cycle * 60 * 1000*1000;
                    return E_SUCCESS;
                }
            ...
            {                
                ...
                else
                {
                    if (m_container_client->exist_docker_image(task->training_engine) != E_SUCCESS)
                    {
                        ...
                        detach_pull_map(task, PULLING_ERROR);
                        return E_DEFAULT;
                    }
                    LOG_DEBUG << "docker pull image success. engine: " << task->training_engine;
                    detach_pull_map(task, PULLING_SUCCESS);
                }
            }
        ...
        int32_t ai_power_provider_service::detach_pull_map(std::shared_ptr<ai_training_task> task, pull_state finished)
        {
            if (m_pull_image_map.find(task->training_engine) == m_pull_image_map.end())
            {
                return E_SUCCESS;
        ...
        {
            if (m_pull_image_map.find(task->task_id) != m_pull_image_map.end())
            {
            ...
            else
            {
                ...
                //try start, if image does not exist, then pull image
                start_exec_training_task(task);                
            }
