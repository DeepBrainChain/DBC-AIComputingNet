...
namespace ai
{
    namespace dbc
    {
        ...
        int32_t ai_power_provider_service::on_start_training_req(std::shared_ptr<message> &msg)
        {
            ...
            if (m_user_task_ptr->get_user_task_size() >= AI_TRAINING_MAX_TASK_COUNT)
            {
                LOG_ERROR << "ai power provider service on start training too many tasks, task id: " << req->body.task_id;
                return E_DEFAULT;
            }

            if (m_user_task_ptr->have_task(req->body.task_id))
            {
        ...
        int32_t ai_power_provider_service::on_stop_training_req(std::shared_ptr<message> &msg)
        {
            ...
            //check task_id
            if (0 == m_user_task_ptr->get_user_task_size())
            {
        ...
        int32_t ai_power_provider_service::on_logs_req(const std::shared_ptr<message> &msg)
        {
            ...
            //check task id
            if (! m_user_task_ptr->have_task(task_id))
            {
        ...
        int32_t ai_power_provider_service::on_training_task_timer(std::shared_ptr<core_timer> timer)
        {
            ...
            if (0 == m_user_task_ptr->get_user_task_size() && nullptr != m_idle_task_ptr)
            {
            ...
            auto task = m_user_task_ptr->get_top_task();
            ...
            {
                m_user_task_ptr->check_pull_image_state();
                return E_SUCCESS;
            }
            else if (task_running == task->status)
            {
                ...
                return m_user_task_ptr->check_training_task_status();
            }
            else
            {
                //fetch next task to run
                m_user_task_ptr->fetch_nex_task();
        ...
        int32_t ai_power_provider_service::start_exec_training_task(std::shared_ptr<ai_training_task> task)
        {
            ...
            {
                m_user_task_ptr->stop_task(task, task_abnormally_closed);
                m_user_task_ptr->fetch_nex_task();
            ...
            m_oss_task_mng->reset_auth_interval();
            if (E_SUCCESS == m_oss_task_mng->auth_task(task))
            {                    
            ...
            else
            {
                ...
                m_user_task_ptr->stop_task(task, task_overdue_closed);
                return E_SUCCESS;
            }
            ...
            {
                m_idle_task_ptr->stop_task();
            }

            return m_user_task_ptr->exec_task();
        }
        ...
        int32_t ai_power_provider_service::on_get_task_queue_size_req(std::shared_ptr<message> &msg)
        {
            ...
            resp->set(m_user_task_ptr->get_user_task_size());
