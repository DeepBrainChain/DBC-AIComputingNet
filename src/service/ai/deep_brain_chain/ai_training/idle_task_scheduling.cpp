...
namespace ai
{
    namespace dbc
    {
            ...
            try
            {
                ...
                for (it->SeekToFirst(); it->Valid(); it->Next())
                {
                    ...
                    if (! m_idle_task->training_engine.empty()
                        && E_IMAGE_NOT_FOUND == m_container_client->exist_docker_image(m_idle_task->training_engine))
                    {
                        pull_image();
                    }
        ...
        int32_t idle_task_scheduling::exec_task(get_idle_task_container_config get_config)
        {
            ...
            if (E_IMAGE_NOT_FOUND == m_container_client->exist_docker_image(m_idle_task->training_engine))
            {
                pull_image();
                return E_SUCCESS;
            }
        ...
        int32_t idle_task_scheduling::delete_task()
        {
            ...
            {
                LOG_ERROR << "delete task db abnormal  exception";
                return E_DEFAULT;
            }
        ...
        {
            if (nullptr == m_idle_task || m_idle_task->task_id.empty())
            {
                return IDLE_TASK_NULL;
            }
            //inspect container
            std::shared_ptr<container_inspect_response> resp = m_container_client->inspect_container(m_idle_task->task_id);
            if (nullptr == resp)
            {
            ...
            if (!resp->id.empty() && m_idle_task->container_id.empty())
            {
        ...
        void idle_task_scheduling::set_task(std::shared_ptr<idle_task_resp> task)
        {
            ...
            else if (m_idle_task->task_id != task->idle_task_id)
            {
                ...
                if (E_IMAGE_NOT_FOUND == m_container_client->exist_docker_image(task->training_engine))
                {
                    pull_image();
                }
                write_task_to_db();
        ...
        int32_t idle_task_scheduling::pull_image()
        {
            if (nullptr == m_idle_task || m_idle_task->training_engine.empty())
            {
                return E_SUCCESS;
            }
            ...
            if (m_pull_image_mng->get_pull_image_name() != m_idle_task->training_engine)
            {
                if (PULLING == m_pull_image_mng->check_pull_state())
                {
                    m_pull_image_mng->terminate_pull();
                }
                m_pull_image_mng->start_pull(m_idle_task->training_engine);
                return E_SUCCESS;
            }
            ...
            //start pulling
            if (E_SUCCESS != m_pull_image_mng->start_pull(m_idle_task->training_engine))
            {
                LOG_ERROR << "idle task engine pull fail. engine:" << m_idle_task->training_engine;
