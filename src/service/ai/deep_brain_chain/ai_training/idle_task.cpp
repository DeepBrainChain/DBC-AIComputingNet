...
namespace ai
{
    namespace dbc
    {
        idle_task::idle_task(std::shared_ptr<container_client> & container_client_ptr)
        {
            m_container_client = container_client_ptr;
            init_db();
        }
        
        int32_t idle_task::init_db()
        {
        ...
        int32_t idle_task::write_task_to_db()
        {
            //serialization
        ...
        int32_t idle_task::create_task(get_idle_task_container_config get_config)
        {
            if (nullptr == m_idle_task)
            {
                return E_DEFAULT;
        ...
        int32_t idle_task::exec_task(get_idle_task_container_config get_config)
        {
            if (nullptr == m_idle_task)
            {
                return E_SUCCESS;
        ...
        idle_task_state idle_task::get_task_state()
        {
            if (nullptr == m_idle_task || m_idle_task->container_id.empty())
            {
        ...
        void idle_task::set_task(std::shared_ptr<idle_task_resp> task)
        {
            if (nullptr == m_idle_task)
            {
                m_idle_task = std::make_shared<ai_training_task>();
