...
namespace ai
{
    namespace dbc
    {
        ...
        int32_t user_task_scheduling::load_task()
        {
            try
            {
                ...
                for (it->SeekToFirst(); it->Valid(); it->Next())
                {
                    ...
                    LOG_DEBUG << "user task scheduling insert ai training task to task map, task id: " << task->task_id << " container_id: " << task->container_id << " task status: " << task->status;
                    ...
                    if (task_running == task->status || task_queueing == task->status || task_pulling_image == task->status)
                    {
                        ...
                        LOG_DEBUG << "user task scheduling insert ai training task to task queue, task id: " << task->task_id << " container_id: " << task->container_id << " task status: " << task->status;
                    }
                }
        ...
        //if the task have been cached ,return true.
        bool user_task_scheduling::have_task(std::string task_id)
        {
            ...
            if (m_training_tasks.count(task_id) != 0)
            {
                return true;
            }
