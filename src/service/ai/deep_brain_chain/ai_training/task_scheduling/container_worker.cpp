...
namespace ai
{
    namespace dbc
    {
        ...
        */
        ...
        {
            prune_container(m_prune_container_interval);
            return E_SUCCESS;
        }

        int32_t container_worker::prune_container(int16_t interval)
        {
            int16_t prune_interval = interval;
            //get docker root dir
            if (m_docker_root_dir.empty())
            {
                std::shared_ptr<docker_info> docker_info_ptr = m_container_client->get_docker_info();
            ...
            //if prune_interval=0, the containers were pruned all.
            m_container_client->prune_container(prune_interval);

            //when free space is not enough, even prune the container, the space maybe not enough too. so need to exec prune again.            
            if (prune_interval != interval && prune_interval != 0)
            {
                prune_container(prune_interval);
            }
