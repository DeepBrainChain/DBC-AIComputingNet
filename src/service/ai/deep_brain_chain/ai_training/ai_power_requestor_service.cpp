...
namespace ai {
    namespace dbc {
        ...
        {
            LOG_INFO << "on_cmd_start_training_req";
            ...
            std::shared_ptr<cmd_start_training_req> req_msg = std::dynamic_pointer_cast<cmd_start_training_req>(msg_content);

            if (nullptr != req_msg) {
                cmd_resp->header.__set_session_id(req_msg->header.session_id);
            }

            if (msg_content == nullptr || req_msg == nullptr)
            {
                cmd_resp->result = E_DEFAULT;
            ...
            auto task_req_msg = create_task_msg(req_msg->vm, cmd_resp->task_info);
            ...
            std::string code_dir=req_msg->vm["code_dir"].as<std::string>();
            ...
            else
            {
                ...
                else
                {
                    ...
                    try
                    {
                        std::vector<std::string> nodes = req_msg->vm["peer_nodes_list"].as<std::vector<std::string>>();
                    ...
                    task_description += " : " + req_msg->parameters["description"];
