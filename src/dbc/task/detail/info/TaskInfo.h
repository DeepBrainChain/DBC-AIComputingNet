...
public:
    ...
    TaskInfo() { m_db_info = std::make_shared<dbc::db_task_info>(); }
    ...
    std::string getRenterWallet() const {
        RwMutex::ReadLock rlock(m_mtx);
        return m_db_info->wallet;
    }

    void setRenterWallet(const std::string& wallet) {
        RwMutex::WriteLock wlock(m_mtx);
        m_db_info->__set_wallet(wallet);
    }
