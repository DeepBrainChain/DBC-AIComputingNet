...
public:
    ...
    // 添加新数据盘
    FResult addNewDisk(const std::string& task_id, int64_t size_k, const std::string& mount_dir = "/data");
    ...
    // 获取某个快照
    bool getSnapshot(const std::string& task_id, const std::string& snapshot_name, dbc::snapshot_info& snapshot);
