...
struct CreateTaskParams {
    ...
    // 操作系统(如generic, ubuntu 18.04, windows
    // 10)，默认ubuntu，带有win则认为是windows系统，必须全小写。
...
public:
    ...
    FResult shutdownTask(const std::string& wallet,
                         const std::string& rent_order,
                         const std::string& task_id);

    FResult poweroffTask(const std::string& wallet,
                         const std::string& rent_order,
                         const std::string& task_id);

    FResult restartTask(const std::string& wallet,
                        const std::string& rent_order,
                        const std::string& task_id, bool force_reboot = false);

    FResult resetTask(const std::string& wallet, const std::string& rent_order,
                      const std::string& task_id);

    FResult deleteTask(const std::string& wallet, const std::string& rent_order,
                       const std::string& task_id);
    ...
    FResult getTaskLog(const std::string& task_id,
                       QUERY_LOG_DIRECTION direction, int32_t nlines,
                       std::string& log_content);

    void listAllTask(const std::string& wallet,
                     std::vector<std::shared_ptr<TaskInfo>>& vec);

    void listRunningTask(const std::string& wallet,
                         std::vector<std::shared_ptr<TaskInfo>>& vec);

    std::shared_ptr<TaskInfo> findTask(const std::string& wallet,
                                       const std::string& task_id);
    ...
    FResult downloadImage(
        const std::string& wallet,
        const std::shared_ptr<dbc::node_download_image_req_data>& data);

    FResult downloadImageProgress(
        const std::string& wallet,
        const std::shared_ptr<dbc::node_download_image_progress_req_data>&
            data);

    FResult stopDownloadImage(
        const std::string& wallet,
    ...
    // snapshot
    FResult listTaskSnapshot(const std::string& wallet,
                             const std::string& task_id,
                             std::vector<dbc::snapshot_info>& snapshots);

    FResult createSnapshot(const std::string& wallet,
                           const std::string& task_id,
                           const std::string& snapshot_name,
                           const ImageServer& imgsvr, const std::string& desc);

    FResult terminateSnapshot(const std::string& wallet,
                              const std::string& task_id,
                              const std::string& snapshot_name);

    // network
    int32_t getTaskAgentInterfaceAddress(
        const std::string& task_id,
...
protected:
    ...
    void shell_remove_iptable_from_system(const std::string& task_id,
                                          const std::string& host_ip,
                                          uint16_t ssh_port,
                                          const std::string& task_local_ip,
                                          const std::string& public_ip);

    void shell_add_iptable_to_system(
        const std::string& task_id, const std::string& host_ip,
    ...
    FResult check_public_ip(const std::string& public_ip,
                            const std::string& task_id = "");
    ...
    FResult check_port_conflict(uint16_t port,
    ...
    bool allocate_gpu(int32_t gpu_count,
                      std::map<std::string, std::list<std::string>>& gpus,
                      std::vector<int32_t> gpu_order,
                      const std::string& exclude_task_id = "");
    ...
    static void getNeededBackingImage(const std::string& image_name,
                                      std::vector<std::string>& backing_images);
