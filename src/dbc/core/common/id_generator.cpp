...
namespace matrix
{
    namespace core {
        namespace id_generator {
            ...
            bool decode_private_key(const std::string &node_privarte_key, std::vector<uint8_t> &vch) {
                ...
                for (it_prfix = private_key_prefix.begin(), it_key = vch.begin();
                     it_prfix != private_key_prefix.end() && it_key != vch.end();) {
                    if (*it_prfix != *it_key) {
                        return false;
                    }
