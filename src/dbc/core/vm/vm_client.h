...
namespace matrix
{
    ...
    { 
        ...
        public:
            ...
            int32_t rename_vm(std::string name,std::string autodbcimage_version) { return 0; }
            int32_t restart_vm(std::string vm_id);
            int32_t update_vm(std::string vm_id, std::shared_ptr<update_vm_config> config);
            std::string get_commit_image(std::string vm_id,std::string version,std::string task_id,int32_t sleep_time) { return ""; }
            //start
            int32_t start_vm(std::string vm_id) { return 0; }

            int32_t start_vm(std::string vm_id, std::shared_ptr<vm_host_config> config) { return 0; }
            ...
            //wait
            int32_t wait_vm(std::string vm_id) { return 0; }

            //remove
            int32_t remove_vm(std::string vm_id) { return 0; }
            ...
            bool  can_delete_image(std::string image_id) { return true; }
            //logs
            std::shared_ptr<vm_logs_resp> get_vm_log(std::shared_ptr<vm_logs_req> req);
            std::string get_image_id(std::string vm_id) { return ""; }
            int32_t remove_image(std::string image_id);
            void set_address(std::string remote_ip, uint16_t remote_port);

            int32_t exist_docker_image(const std::string & image_name,int32_t sleep_time) { return 0; }

            std::shared_ptr<docker_info> get_docker_info() { return nullptr; }
            ...
            std::string commit_image(std::string vm_id,std::string version,std::string task_id,int32_t sleep_time) { return ""; }
            int32_t exist_vm_time(const std::string & vm_name,int32_t sleep_time) { return 0; }
            std::string get_vm_size(std::string id) { return ""; }
            std::string get_running_vm_no_size() { return ""; }
            std::string get_vm_id_current(std::string task_id) { return ""; }
            std::string get_vm_size_by_task_id(std::string id) { return ""; }
            std::string get_images_original() { return ""; }
            std::string get_gpu_id(std::string vm_id);
            std::string get_docker_dir(std::string vm_id) { return ""; }
        protected:
