...
namespace matrix
{
    namespace core
    {
        ...
        int32_t conf_manager::serialize_node_info(const dbc::machine_node_info &info)
        {
            FILE *fp = nullptr;
            ...
            try
            {
                ...
                if (!fs::is_directory(node_dat_path))
                {
        ...
        int32_t conf_manager::gen_new_nodeid()
        {
            ...
            int32_t ret = dbc::create_node_info(info);                 //check: if exists, not init again and print promption.
            if (E_SUCCESS != ret)
            {
        ...
        {
            std::string node_derived = dbc::derive_pubkey_by_privkey(m_node_private_key);
