...
service_module::~service_module() { this->exit(); }
...
ERRCODE service_module::init() {
    ...
    topic_manager::instance().subscribe(
        ...
        [this](std::shared_ptr<network::message>& msg) { send(msg); });
...
void service_module::send(const std::shared_ptr<network::message>& msg) {
    ...
    if (m_msg_queue.size() < MAX_MSG_QUEUE_SIZE) {
        ...
        LOG_ERROR << "service module message queue is full when add "
                  << msg->get_name() << "message";
        return;
    }
...
void service_module::on_msg_handle(std::shared_ptr<network::message>& msg) {
    ...
    if (msg_name == TIMER_TICK_NOTIFICATION) {
        std::shared_ptr<time_tick_notification> content =
            std::dynamic_pointer_cast<time_tick_notification>(
                msg->get_content());
...
uint32_t service_module::add_timer(
    const std::string& name, uint32_t delay, uint32_t period,
    uint64_t repeat_times, const std::string& session_id,
    const std::function<void(const std::shared_ptr<core_timer>&)>& handle) {
    ...
    std::shared_ptr<core_timer> timer(
        new core_timer(name, delay, period, repeat_times, session_id));
    if (MAX_TIMER_ID == m_timer_alloc_id) {
        LOG_ERROR << "can not allocate new timer id";
        return INVALID_TIMER_ID;
    }
    timer->set_timer_id(
        ++m_timer_alloc_id);  // timer id begins from 1, 0 is invalid timer id
...
void service_module::remove_timer(uint32_t timer_id) {
    ...
    if (!name.empty()) m_timer_invokers.erase(name);
}
...
void service_module::reg_msg_handle(
    const std::string& msgname,
    const std::function<void(const std::shared_ptr<network::message>&)>&
        handle /* = nullptr */) {
    if (handle != nullptr) {
...
int32_t service_module::add_session(
    const std::string& session_id,
    const std::shared_ptr<service_session>& session) {
    RwMutex::WriteLock wlock(m_session_lock);
...
std::shared_ptr<service_session> service_module::get_session(
    const std::string& session_id) {
