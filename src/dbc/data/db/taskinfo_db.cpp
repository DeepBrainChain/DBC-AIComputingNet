...
void TaskInfoDB::load_tasks(std::map<std::string, std::shared_ptr<dbc::TaskInfo>>& tasks)
{
...
bool TaskInfoDB::delete_task(const std::string& task_id)
{
    if (task_id.empty())
        return true;
...
std::shared_ptr<dbc::TaskInfo> TaskInfoDB::read_task(const std::string& task_id) {
    if (task_id.empty())
        return nullptr;
...
bool TaskInfoDB::write_task(const std::shared_ptr<dbc::TaskInfo>& task)
{
