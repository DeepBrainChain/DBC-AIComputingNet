...
ERRCODE ConfManager::Init() {
    ...
    dbclog::instance().set_filter_level(
...
std::string ConfManager::GetNodePrivateKey(const std::string& node_id) const {
    if (node_id.empty() || node_id == m_node_id) return m_node_private_key;
...
ERRCODE ConfManager::ParseConf() {
    ...
    const boost::filesystem::path& conf_path =
        EnvManager::instance().get_conf_file_path();
    try {
    ...
    m_net_flag =
        strtoul(core_args["net_flag"].as<std::string>().c_str(), nullptr, 16);
    ...
    if (core_args.count("dbc_chain_domain") > 0) {
        m_dbc_chain_domain =
            core_args["dbc_chain_domain"].as<std::vector<std::string>>();
    }

    if (core_args.count("chain_additional_field") > 0) {
        m_chain_additional_field =
            core_args["chain_additional_field"].as<std::string>();
    }

    if (core_args.count("image_server") > 0) {
        std::vector<std::string> vec =
            core_args["image_server"].as<std::vector<std::string>>();
        for (auto& str : vec) {
    ...
    if (core_args.count("dbc_monitor_server") > 0) {
        m_dbc_monitor_server =
            core_args["dbc_monitor_server"].as<std::string>();
    }

    if (core_args.count("miner_monitor_server") > 0) {
        m_miner_monitor_server =
            core_args["miner_monitor_server"].as<std::string>();
    }
    ...
    peer_opts.add_options()("peer", bpo::value<std::vector<std::string>>(), "");

    const boost::filesystem::path& peer_path =
        EnvManager::instance().get_peer_file_path();
    try {
...
ERRCODE ConfManager::ParseDat() {
    boost::filesystem::path node_dat_file_path =
        EnvManager::instance().get_dat_file_path();
    if (!boost::filesystem::exists(node_dat_file_path) ||
        boost::filesystem::is_empty(node_dat_file_path)) {
    ...
    try {
        ...
        bpo::store(bpo::parse_config_file(node_dat_ifs, node_dat_opts),
                   node_dat_args);
...
ERRCODE ConfManager::CreateNewNodeId() {
    ...
    if (ERR_SUCCESS != ret) {
        LOG_ERROR << "dbc node info serialization failed: node_id="
                  << info.node_id;
        return ret;
    }
...
ERRCODE ConfManager::SerializeNodeInfo(const util::machine_node_info& info) {
    ...
    try {
        ...
        if (!boost::filesystem::exists(node_dat_path)) {
            LOG_DEBUG
                << "dat directory path does not exist and create dat directory";
...
bool ConfManager::CheckNodeId() {
    std::string node_derived =
        util::derive_pubkey_by_privkey(m_node_private_key);
