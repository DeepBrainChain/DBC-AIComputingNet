...
struct networkInfo {
    ...
    long long rx_bytes;          // 发送的字节数
    long long rx_packets;        // 发送的包
    long long rx_errs;
    long long rx_drop;
    long long tx_bytes;          // 接收的字节数
    long long tx_packets;        // 接收的包
    long long tx_errs;
    long long tx_drop;
    float rx_speed;              // 发送速度，单位B/s? 不确定是否计算
    float tx_speed;              // 接收速度，单位B/s? 不确定是否计算
    struct timespec realTime;    // 实际时间
    // unsigned int delay;
