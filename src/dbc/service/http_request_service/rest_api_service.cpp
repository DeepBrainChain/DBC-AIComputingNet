...
void rest_api_service::init_timer() {
    ...
    m_timer_invokers[NODE_LIST_LAN_TIMER] = std::bind(&rest_api_service::on_node_list_lan_timer, this, std::placeholders::_1);
    m_timer_invokers[NODE_CREATE_LAN_TIMER] = std::bind(&rest_api_service::on_node_create_lan_timer, this, std::placeholders::_1);
    m_timer_invokers[NODE_DELETE_LAN_TIMER] = std::bind(&rest_api_service::on_node_delete_lan_timer, this, std::placeholders::_1);
}
...
void rest_api_service::init_invoker() {
    ...
    m_invokers[NODE_LIST_LAN_RSP] = std::bind(&rest_api_service::on_call_rsp_handler, this, std::placeholders::_1);
    m_invokers[NODE_CREATE_LAN_RSP] = std::bind(&rest_api_service::on_call_rsp_handler, this, std::placeholders::_1);
    m_invokers[NODE_DELETE_LAN_RSP] = std::bind(&rest_api_service::on_call_rsp_handler, this, std::placeholders::_1);
}
...
void rest_api_service::init_subscription() {
    ...
    SUBSCRIBE_BUS_MESSAGE(NODE_LIST_LAN_RSP)
    SUBSCRIBE_BUS_MESSAGE(NODE_CREATE_LAN_RSP)
    SUBSCRIBE_BUS_MESSAGE(NODE_DELETE_LAN_RSP)
}
...
void rest_api_service::rest_list_lan(const std::shared_ptr<dbc::network::http_request> &httpReq, const std::string &path) {
    ...
    if (body.peer_nodes_list.empty()) {
        ...
        // create session
        if (ERR_SUCCESS != create_request_session(NODE_LIST_LAN_TIMER, httpReq, node_req_msg, head_session_id, body.peer_nodes_list[0])) {
            LOG_ERROR << "create request session failed";
            httpReq->reply_comm_rest_err(HTTP_BADREQUEST, RPC_RESPONSE_ERROR, "creaate request session failed");
            return;
        }
...
void rest_api_service::rest_create_lan(const std::shared_ptr<dbc::network::http_request> &httpReq, const std::string &path) {
    ...
    if (ERR_SUCCESS != create_request_session(NODE_CREATE_LAN_TIMER, httpReq, node_req_msg, head_session_id, body.peer_nodes_list[0])) {
        LOG_ERROR << "create request session failed";
        httpReq->reply_comm_rest_err(HTTP_BADREQUEST, RPC_RESPONSE_ERROR, "creaate request session failed");
        return;
    }
...
void rest_api_service::rest_delete_lan(const std::shared_ptr<dbc::network::http_request> &httpReq, const std::string &path) {
    ...
    if (body.peer_nodes_list.empty()) {
        ...
        if (ERR_SUCCESS != create_request_session(NODE_DELETE_LAN_TIMER, httpReq, node_req_msg, head_session_id, body.peer_nodes_list[0])) {
            LOG_ERROR << "create request session failed";
            httpReq->reply_comm_rest_err(HTTP_BADREQUEST, RPC_RESPONSE_ERROR, "creaate request session failed");
            return;
        }
