...
enum HitNodeType { HitNone, HitComputer, HitBareMetal, HitBareMetalManager };

class node_request_service : public service_module,
                             public Singleton<node_request_service> {
public:
...
protected:
    ...
    void task_list(const network::base_header& header,
    ...
    void task_start(const network::base_header& header,
    ...
    void on_node_shutdown_task_req(
    ...
    void on_node_poweroff_task_req(
        const std::shared_ptr<network::message>& msg);
    ...
    void task_stop(const network::base_header& header,
    ...
    void task_reset(const network::base_header& header,
    ...
    void task_logs(const network::base_header& header,
    ...
    void on_node_download_image_req(
        const std::shared_ptr<network::message>& msg);
    ...
    void on_node_download_image_progress_req(
        const std::shared_ptr<network::message>& msg);

    void download_image_progress(
        const network::base_header& header,
        const std::shared_ptr<dbc::node_download_image_progress_req_data>& data,
        const AuthoriseResult& result);

    void on_node_stop_download_image_req(
        const std::shared_ptr<network::message>& msg);

    void stop_download_image(
        const network::base_header& header,
    ...
    void on_node_upload_image_progress_req(
        const std::shared_ptr<network::message>& msg);

    void upload_image_progress(
        const network::base_header& header,
        const std::shared_ptr<dbc::node_upload_image_progress_req_data>& data,
        const AuthoriseResult& result);

    void on_node_stop_upload_image_req(
        const std::shared_ptr<network::message>& msg);

    void stop_upload_image(
        const network::base_header& header,
    ...
    // snapshot
    void on_node_list_snapshot_req(
        const std::shared_ptr<network::message>& msg);
    ...
    void on_node_create_snapshot_req(
        const std::shared_ptr<network::message>& msg);
    ...
    void on_node_delete_snapshot_req(
        const std::shared_ptr<network::message>& msg);
    ...
    void do_list_disk(const network::base_header& header,
    ...
    void do_add_disk(const network::base_header& header,
    ...
    // node
    void on_node_query_node_info_req(
        const std::shared_ptr<network::message>& msg);
    ...
    void query_bare_metal_node_info(
        const network::base_header& header,
        const std::shared_ptr<dbc::node_query_node_info_req_data>& data);

    // query node rent orders
    void on_query_node_rent_orders_req(
        const std::shared_ptr<network::message>& msg);

    void query_node_rent_orders(
        const network::base_header& header,
    ...
    std::shared_ptr<network::message> create_service_broadcast_req_msg(
        const std::map<std::string, std::shared_ptr<dbc::node_service_info>>&
            mp);

    void on_net_service_broadcast_req(
        const std::shared_ptr<network::message>& msg);

    // monitor
    void on_node_list_monitor_server_req(
        const std::shared_ptr<network::message>& msg);

    void monitor_server_list(
        const network::base_header& header,
        const std::shared_ptr<dbc::node_list_monitor_server_req_data>& data,
        const AuthoriseResult& result);

    void on_node_set_monitor_server_req(
        const std::shared_ptr<network::message>& msg);

    void monitor_server_set(
        const network::base_header& header,
    ...
    void delete_lan(const network::base_header& header,
    ...
    // delete bare metal
    void on_node_delete_bare_metal_req(
        const std::shared_ptr<network::message>& msg);

    void delete_bare_metal(
        const network::base_header& header,
...
private:
    ...
    HitNodeType hit_node(const std::vector<std::string>& peer_node_list,
                         const std::string& node_id);

    FResult check_nonce(const std::string& wallet, const std::string& nonce,
    ...
    std::tuple<std::string, std::string> parse_wallet(
        const AuthorityParams& params);

    void check_authority(const AuthorityParams& params,
