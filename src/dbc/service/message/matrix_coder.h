...
namespace matrix {
    namespace service_core {
        ...
        class matrix_coder : public message_to_byte_encoder, public length_field_frame_decoder {
            using decode_invoker_type = typename std::function<void(shared_ptr<message> &, base_header &,
                                                                    shared_ptr<protocol> &)>;
            using encode_invoker_type = typename std::function<void(channel_handler_context &,
                                                                    std::shared_ptr<protocol> &, message &,
                                                                    byte_buf &)>;
        public:
            matrix_coder();
            ...
            encode_status encode(channel_handler_context &ctx, message &msg, byte_buf &out) override;

            decode_status recv_message(byte_buf &in) override;

            decode_status decode_frame(channel_handler_context &ctx, byte_buf &in, std::shared_ptr<message> &msg) override;
        ...
        protected:
            ...
            decode_status
            decode_service_frame(channel_handler_context &ctx, byte_buf &in, std::shared_ptr<message> &msg,
                                 std::shared_ptr<protocol> proto);

            decode_status decode_fast_forward(channel_handler_context &ctx, byte_buf &in, std::shared_ptr<message> &msg,
                                              base_header &header, std::shared_ptr<protocol> proto);

            template<typename msg_type>
            void
            decode_invoke(std::shared_ptr<message> &msg, base_header &msg_header, std::shared_ptr<protocol> &proto);

            template<typename msg_type>
            void
            encode_invoke(channel_handler_context &ctx, std::shared_ptr<protocol> &proto, message &msg, byte_buf &out);
