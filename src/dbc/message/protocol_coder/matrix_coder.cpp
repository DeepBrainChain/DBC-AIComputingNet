...
matrix_coder::matrix_coder()
    : length_field_frame_decoder(MATRIX_MSG_MIN_READ_LENGTH,
                                 MAX_MATRIX_MSG_LEN) {
    init_decode_proto();
...
void matrix_coder::init_decode_proto() {
    m_decode_protos[THRIFT_BINARY_PROTO] =
        std::make_shared<network::binary_protocol>();
    m_decode_protos[THRIFT_COMPACT_PROTO] =
        std::make_shared<network::compact_protocol>();
}
...
void matrix_coder::init_decode_invoker() {
    ...
    BIND_DECODE_INVOKER(node_list_deeplink_info_req)
    BIND_DECODE_INVOKER(node_list_deeplink_info_rsp)

    BIND_DECODE_INVOKER(node_set_deeplink_info_req)
    BIND_DECODE_INVOKER(node_set_deeplink_info_rsp)
}
...
void matrix_coder::init_encode_invoker() {
    ...
    BIND_ENCODE_INVOKER(node_list_deeplink_info_req)
    BIND_ENCODE_INVOKER(node_list_deeplink_info_rsp)

    BIND_ENCODE_INVOKER(node_set_deeplink_info_req)
    BIND_ENCODE_INVOKER(node_set_deeplink_info_rsp)
}
...
/**
 * decode input message as an binary forward message if it match the fast
 ...
 */
decode_status matrix_coder::decode_fast_forward(
    network::channel_handler_context &ctx, byte_buf &in,
    ...
    if (find(legacy_none_forwarding_messages.begin(),
    ...
    if (header.exten_info.count("dest_id") &&
        ...
        std::shared_ptr<network::msg_forward> content =
            std::make_shared<network::msg_forward>();
        ...
        content->body.write_to_byte_buf(buf->get_read_ptr(),
                                        buf->get_valid_read_len());
...
decode_status matrix_coder::decode_frame(
    network::channel_handler_context &ctx, byte_buf &in,
    ...
    try {
        ...
        // get decode protocol
        std::shared_ptr<network::protocol> proto =
            get_protocol(packet_header.packet_type & 0xff);
        if (nullptr == proto) {
            return DECODE_ERROR;
        } else {
        ...
        decode_status decodeRet =
            decode_service_frame(ctx, decode_in, msg, proto);
        if (ERR_SUCCESS == decodeRet) {
            int32_t framelen =
                before_decode_len - decode_in.get_valid_read_len();
            if (packet_header.packet_len != framelen) {
                LOG_ERROR << "matrix msg_len error. msg_len in code frame is: "
                          << packet_header.packet_len
...
decode_status matrix_coder::decode_service_frame(
    network::channel_handler_context &ctx, byte_buf &in,
    std::shared_ptr<network::message> &msg,
    std::shared_ptr<network::protocol> proto) {
    // service header
    ...
    if (it == m_binary_decode_invokers.end()) {
        LOG_ERROR << "matrix decoder received unknown message: "
                  << header.msg_name;
        // return DECODE_ERROR;
...
void matrix_coder::decode_invoke(std::shared_ptr<network::message> &msg,
                                 network::base_header &header,
                                 std::shared_ptr<network::protocol> &proto) {
    ...
    // LOG_DEBUG << "matrix coder decode message: " << content->header.msg_name
    // << ", message length: ";
}

encode_status matrix_coder::encode(network::channel_handler_context &ctx,
    ...
    try {
        ...
        if (proto == nullptr) {
            LOG_ERROR << "matrix encoder unknown protocol value: "
                      << thrift_proto;
            return ENCODE_ERROR;
        }
        ...
        // encode
        std::shared_ptr<network::msg_forward> bin_fwd_content =
        ...
        if (bin_fwd_content) {
            ...
            proto->append_raw(bin_fwd_content->body.get_read_ptr(),
                              bin_fwd_content->body.get_valid_read_len());

        } else {
            // find encoder
            auto it = m_binary_encode_invokers.find(msg.get_name());
            if (it == m_binary_encode_invokers.end()) {
                LOG_ERROR << "matrix encoder received unknown message: "
                          << msg.get_name();
                return ENCODE_ERROR;
            }
        ...
        if (enable_compress &&
            packet_header.packet_len >=
                network::matrix_compress::MIN_MSG_LEN_TO_COMPRESS) {
...
void matrix_coder::encode_invoke(network::channel_handler_context &ctx,
                                 std::shared_ptr<network::protocol> &proto,
                                 network::message &msg, byte_buf &out) {
    std::shared_ptr<msg_type> content =
        std::dynamic_pointer_cast<msg_type>(msg.content);
...
decode_status matrix_coder::recv_message(byte_buf &in) {
    while (in.get_valid_read_len() > 0) {
        ...
        if (msg.has_data() && (msg.get_packetlen() > MAX_MATRIX_MSG_LEN ||
                               msg.get_packetlen() < MIN_MATRIX_MSG_CODE_LEN)) {
...
std::string matrix_coder::get_local_node_id(
    network::channel_handler_context &ctx) {
    std::string rtn;
...
matrix_packet_header::matrix_packet_header() : packet_len(0), packet_type(0) {}
...
bool matrix_packet_header::update(byte_buf &out) {
    if (packet_len != out.get_valid_read_len() &&
        packet_len != out.get_valid_write_len()) {
