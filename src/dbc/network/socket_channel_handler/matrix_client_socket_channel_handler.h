...
public:
    ...
    matrix_client_socket_channel_handler(std::shared_ptr<dbc::network::channel> ch);

    virtual ~matrix_client_socket_channel_handler() = default;

    virtual int32_t start();

    static std::shared_ptr<socket_channel_handler> create(std::shared_ptr<dbc::network::channel> ch);

    virtual int32_t on_after_msg_received(dbc::network::message &msg);

    virtual int32_t on_after_msg_sent(dbc::network::message &msg);
