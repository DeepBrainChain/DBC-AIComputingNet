...
inline raii_evhttp_request obtain_evhttp_request(
    void (*cb)(struct evhttp_request *, void *), void *arg) {
    return raii_evhttp_request(evhttp_request_new(cb, arg));
}

inline raii_evhttp_connection obtain_evhttp_connection_base(
    struct event_base *base, std::string host, uint16_t port) {
...
inline raii_evhttp_connection obtain_evhttp_connection_base2(
    struct event_base *base, struct bufferevent *bev, std::string host,
    uint16_t port) {
    auto result = raii_evhttp_connection(evhttp_connection_base_bufferevent_new(
        base, nullptr, bev, host.c_str(), port));
    if (!result.get()) throw std::runtime_error("create connection failed");
    return result;
}
...
inline raii_evhttp_uri obtain_evhttp_uri_parse(const std::string &url) {
    ...
    if (!result.get()) throw std::runtime_error("parse url error");
    return result;
}
...
public:
    ...
    int32_t post(const std::string &endpoint, const kvs &headers,
                 const std::string &req_content, http_response &resp);
    int32_t post_sleep(const std::string &endpoint, const kvs &headers,
                       const std::string &req_content, http_response &resp,
                       int32_t sleep_time);

    int32_t get(const std::string &endpoint, const kvs &headers,
                http_response &resp);
    int32_t get_sleep(const std::string &endpoint, const kvs &headers,
                      http_response &resp, int32_t sleep_time);
    int32_t del(const std::string &endpoint, const kvs &headers,
                http_response &resp);
