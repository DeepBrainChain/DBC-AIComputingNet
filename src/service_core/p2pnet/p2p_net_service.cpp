    ...
    namespace service_core {
        ...
        void p2p_net_service::init_subscription() {
            ...
            TOPIC_MANAGER->subscribe(CLIENT_CONNECT_NOTIFICATION,
                                     [this](std::shared_ptr<message> &msg) { return send(msg); });
        ...
        int32_t p2p_net_service::on_tcp_channel_error(std::shared_ptr<message> &msg) {
            ...
            std::shared_ptr<tcp_socket_channel_error_msg> err_msg = std::dynamic_pointer_cast<tcp_socket_channel_error_msg>(
                    msg);
            if (!err_msg) {
        ...
        int32_t p2p_net_service::send_put_peer_nodes(std::shared_ptr<peer_node> node) {
            ...
            } else// broadcast all nodes
            {
                int count = 0;
                for (auto it = m_peer_nodes_map.begin(); it != m_peer_nodes_map.end(); ++it) {
                    if (nullptr == it->second || SERVER_SOCKET == it->second->m_sid.get_type() ||
